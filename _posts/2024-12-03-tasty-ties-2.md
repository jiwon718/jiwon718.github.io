...
맛잇다는 번역 서비스를 지원하는 온라인 쿠킹 클래스 웹사이트입니다. 맛잇다에 여러 가지 기능이 있지만, 그 중에서 사용자들 간 다대다 채팅을 담당형 개발하였습니다. 이번에는 그 채팅을 개발하는 과정 중 두 번째는 Spring WebSocket을 활용하여 채팅방을 구분하여 채팅방끼리 채팅하는 것을 구현한 것에 대해서 공유하려고 합니다.

클라이언트가 채팅방 번호를 입력하여 연결하면, 해당 채팅방에 참가자들끼리 채팅하는 것을 구현하겠습니다.

해당 내용은 첫 번째 내용인 [\[맛잇다\] Spring Boot + WebSocket을 활용한 간단한 채팅 구현]에서 이어진 것이므로, 이전 글을 읽으시면 해당 글을 이해하시는 데에 많은 도움이 될 것입니다.

[\[맛잇다\] Spring Boot + WebSocket을 활용한 간단한 채팅 구현]: https://spring.io/guides/gs/messaging-stomp-websocket
...
<figure style="width: 100%" class="half">
    <a href="link"><img src="../assets/img/2024-12-03-tasty-ties-2/변경 전 프로젝트 구조.png" alt="변경 전 프로젝트 구조"></a>
    <a href="link"><img src="../assets/img/2024-12-03-tasty-ties-2/변경 후 프로젝트 구조.png" alt="변경 후 프로젝트 구조"></a>
    <figcaption>변경 전/후 프로젝트 구조</figcaption>
</figure>
...
<span style="color: #D9D7D8">
이는 기능에 아무런 관련이 없지만, 제가 클라이언트가 어떤 주소를 사용해야 메세지를 송수신하는지에 대해 구분하기 위해서 수정하는 것입니다.
따라서, 수정하지 않으셔도 크게 상관없습니다.
</span>
...
<span style="color: #D9D7D8">
controller 이름은 채팅 메세지를 처리하는 것임을 명시하기 위해서 수정했습니다.
</span>
...
이렇게 클라이언트에게 메세지를 전달하는 주소를 `/sub/chat/rooms/{roomId}`로 수정하면 `roomId`별로 메세지가 구부노디어서 다시 클라이언트에게 메세지가 전달됩니다.
...
<span style="color: #D9D7D8">
해당 글은 Spring Boot WebSocket을 활용하여 채팅 서버를 구현하는 것에 초점을 두었기에 프론트와 관련된 코드에 대해서는 생략했습니다. 궁금하신 분들은 맨 아래 첨부된 GitHub 주소를 참고해주세요.
</span>
...
로그를 자세히 살펴보니 다음 내용을 통해 <span style="background-color: #F0ADC2">`ChatMessageResponseDto` JSON 직렬화하는 과정에서 오류가 발생한다</span>는 것을 파악할 수 있었습니다.
...
Spring Boot가 기본적으로 JSON 직렬화/역직렬화할 때 Jackson을 사용하는데, <span style="background-color: #FEEA67">해당 라이브러리는 JSON 직렬화/역직렬화할 때 getter 메소드를 사용한다</span>는 것을 알 수 있었습니다.
...
## 리플렉션
- Java와 같은 프로그래밍 언어에서 <span style="background-color: #FEEA67">런타임 시점에 객체의 클래스, 메소드, 필드 등 구조적인 정보를 분석하고 수정할 수 있는 기능을 제공</span>합니다.
...
- <span style="background-color: #F0ADC2">`private` 필드나 메소드에 접근할 수 있기 때문에 보안 문제가 발생</span>할 수 있습니다.
- <span style="background-color: #F0ADC2">컴파일 타임에 오류를 잡을 수 없기 때문에 런타임 오류가 발생</span>할 수 있습니다.
...
JSON 역직렬화하는 과정에서 발생한 오류를 해결하면서 리플렉션이라는 것을 알 수 있습니다.  
